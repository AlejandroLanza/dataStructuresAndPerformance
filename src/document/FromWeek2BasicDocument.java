...
//		System.out.println("Inside getNumWords: " + getTokens("[a-z,A-Z]+"));
	...
	public int getNumSyllables()
	{
		...
		int count = 0;
		// get text's words separated using getTokens, then get a count for each word from countSyllables and add it up!
		for(String myWord : getTokens("[A-Za-z]+")){
			count += countSyllables(myWord);
		};
        return count;
	}
...
//		         32, 15, 1);
		System.out.println("Flesch score for \"hello dudster, whats going on?: \""); 
		BasicDocument myDoc = new BasicDocument("one (1), two (2), three (3)");
		System.out.println("Words: " + myDoc.getNumWords() + " Sentences: " + myDoc.getNumSentences() 
		+ " Syllables: " + myDoc.getNumSyllables());
		System.out.println(myDoc.getFleschScore());
		System.out.println(myDoc.getTokens("[^, ]+"));
//		System.out.println(new BasicDocument("this whateverss. Yeah right! ok then, or not really? your turn.").getTokens("[^.!?]+"));
//		System.out.println(new BasicDocument("yello").countSyllables("Lorem ipsum dolor sit amet, qui ex choro quodsi moderatius, nam dolores explicari forensibus ad."));
	}
